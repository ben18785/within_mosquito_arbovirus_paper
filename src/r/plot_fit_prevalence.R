...
prepare_simulated_data_mcmc <- function(first_index, fit, data_in,
  ...
  midgut_1 <- tibble(
    middle=middles[first_index, , 2, 1],
    lower=lowers[first_index, , 2, 1],
    upper=uppers[first_index, , 2, 1],
    dilutions=1,
    tissue="midgut",
    day=data_in$g_t,
    type="simulated")
  midgut_2 <- tibble(
    middle=middles[first_index, , 2, 2],
    lower=lowers[first_index, , 2, 2],
    upper=uppers[first_index, , 2, 2],
    dilutions=5,
    tissue="midgut",
    day=data_in$g_t,
    type="simulated")
  midgut_3 <- tibble(
    middle=middles[first_index, , 2, 3],
    lower=lowers[first_index, , 2, 3],
    upper=uppers[first_index, , 2, 3],
    dilutions=12,
    tissue="midgut",
    day=data_in$g_t,
    type="simulated")
  midgut_4 <- tibble(
    middle=middles[first_index, , 2, 4],
    lower=lowers[first_index, , 2, 4],
    upper=uppers[first_index, , 2, 4],
    dilutions=20,
    tissue="midgut",
    day=data_in$g_t,
    type="simulated")
  midgut_5 <- tibble(
    middle=middles[first_index, , 2, 5],
    lower=lowers[first_index, , 2, 5],
    upper=uppers[first_index, , 2, 5],
    dilutions=25,
    tissue="midgut",
    day=data_in$g_t,
    type="simulated")
  ...
  legs_1 <- tibble(
    middle=middles[first_index, , 3, 1],
    lower=lowers[first_index, , 3, 1],
    upper=uppers[first_index, , 3, 1],
    dilutions=1,
    tissue="legs",
    day=data_in$g_t,
    type="simulated")
  legs_2 <- tibble(
    middle=middles[first_index, , 3, 2],
    lower=lowers[first_index, , 3, 2],
    upper=uppers[first_index, , 3, 2],
    dilutions=5,
    tissue="legs",
    day=data_in$g_t,
    type="simulated")
  legs_3 <- tibble(
    middle=middles[first_index, , 3, 3],
    lower=lowers[first_index, , 3, 3],
    upper=uppers[first_index, , 3, 3],
    dilutions=12,
    tissue="legs",
    day=data_in$g_t,
    type="simulated")
...
prepare_data_prefit_mcmc <- function(fit, list_stan_datasets) {
  ...
  sim_df_c <- prepare_simulated_data_mcmc(1, fit, data_in) %>% 
    mutate(category="continuous")
  sim_df_d <- prepare_simulated_data_mcmc(2, fit, data_in) %>% 
    mutate(category="dichtonomous")
  sim_df <- sim_df_c %>% 
    bind_rows(sim_df_d)
  ...
  sigma <- tibble(sigma=sigmas,
                  tissue=c("midgut", "legs"))
  dilutions <- c(1, 5, 12, 20, 25)
  
  phi_m <- dose_response_curve_mcmc(dilutions * (1 / zeta), fit)
  phi_1 <- tribble(~phi, ~tissue, ~dilutions,
                   phi_m[1], "midgut", 1,
                   phi_m[2], "midgut", 5,
                   phi_m[3], "midgut", 12,
  ...
  phi_m <- dose_response_curve_mcmc(dilutions, fit)
  phi_2 <- tribble(~phi, ~tissue, ~dilutions,
                   phi_m[1], "midgut", 1,
                   phi_m[2], "midgut", 5,
                   phi_m[3], "midgut", 12,
  ...
  simulated_df <- combined_df %>% 
    ...
    left_join(sigma, by="tissue") %>%
    left_join(phi, by=c("tissue", "dilutions", "category")) %>%
    pivot_wider(values_from = denv_titer,
                names_from = data_type) %>% 
    mutate(
      middle=if_else(middle < 0, 1e-10, middle),
      upper=if_else(upper < 0, 1e-10, upper),
      lower=if_else(lower < 0, 1e-10, lower)
      ) %>% 
    mutate(
      middle= phi * (1-plnorm(threshold, log(middle), sigma)),
      lower= phi * (1-plnorm(threshold, log(lower), sigma)),
      upper= phi * (1-plnorm(threshold, log(upper), sigma))
      ) %>% 
    rename(dilution=dilutions)
...
plot_fit_prevalence_midgut_only_mcmc <- function(fit, list_stan_datasets) {
