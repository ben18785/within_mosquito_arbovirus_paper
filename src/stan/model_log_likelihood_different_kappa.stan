...
data {
  ...
  int dilutions_lookup[n_dilutions]; // dilution values
  ...
  int dilutions_binary_lookup[n_experiment_types_binary];
...
parameters {
  ...
  positive_ordered[5] k_m; // one parameter for each concentration
  real<lower=0> k_mh;
...
transformed parameters {
  ...
  vector[n_obs_binary] log_likelihood_binary;
  vector[n_chp] log_likelihood_chp;
  {
      real theta[n_theta];
      real mu[n_obs];
      ...
      for(i in 1:n_dilutions){
            theta[5] = k_m[dilutions_lookup[i]];
      ...
      for(i in 1:n_experiment_types_binary){
        ...
        theta_temp[5] = k_m[dilutions_binary_lookup[i]];
      ...
      if(include_hurdle == 1) {
        for(i in 1:n_obs) {
          ...
          if(difeq_ind[i] == 3) // legs
            phi *= phi_d;
          if(y[i] > 0){
            log_likelihood[i] = log(phi) + lognormal_lpdf(y[i] | mu[i], sigma_in[i]); // no need for truncation as per: https://mc-stan.org/docs/2_18/stan-users-guide/censored-data.html
          }else {
            real log_prob[2];
            log_prob[1] = log1m(phi);
            log_prob[2] = log(phi) + lognormal_lcdf(titer_lower_bound[difeq_ind[i] - 1]| mu[i], sigma_in[i]);
            log_likelihood[i] = log_sum_exp(log_prob);
          }
        }
      }else {
        for(i in 1:n_obs) {
          if(y[i] >= 0){
            log_likelihood[i] = lognormal_lpdf(y[i] | mu[i], sigma_in[i]);
          } else {
            if(include_continuous_censored == 1)
              log_likelihood[i] = lognormal_lcdf(titer_lower_bound[difeq_ind[i] - 1]| mu[i], sigma_in[i]);
          }
        }
      }
    }
    ...
    if(include_binary == 1) {
      for(i in 1:n_obs_binary) {
        ...
        log_likelihood_binary[i] =  binomial_lpmf(n_infected_binary[i]|n_dissected_binary[i], p);
      }
    }
    ...
    for(i in 1:n_chp) {
      ...
      log_likelihood_chp[i] = lognormal_lpdf(chp[i] | log(mu_tmp), chp_sigma);
    }
  }
}
...
model {
  ...
  // likelihoods
  for(i in 1:n_obs)
    target += log_likelihood[i];
  for(i in 1:n_obs_binary)
    target += log_likelihood_binary[i];
  for(i in 1:n_chp)
    target += log_likelihood_chp[i];
}
